...
export default function PrestartUpdateForm(props) {
  const {
    id: idProp,
    prestart: prestartModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = prestartRecord
      ? { ...initialValues, ...prestartRecord }
      : initialValues;
    setOdometerreading(cleanValues.odometerreading);
    setEnginehoursos(cleanValues.enginehoursos);
    setCategeoryAfault(cleanValues.categeoryAfault);
    setCategeoryBfault(cleanValues.categeoryBfault);
    setCategeoryCfault(cleanValues.categeoryCfault);
    setErrors({});
  };
  const [prestartRecord, setPrestartRecord] = React.useState(prestartModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Prestart, idProp)
        : prestartModelProp;
      setPrestartRecord(record);
    };
    queryData();
  }, [idProp, prestartModelProp]);
  React.useEffect(resetStateValues, [prestartRecord]);
  const validations = {
    odometerreading: [],
    enginehoursos: [],
    categeoryAfault: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Prestart.copyOf(prestartRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PrestartUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || prestartModelProp)}
