...
export default function MetstruckplodUpdateForm(props) {
  const {
    id: idProp,
    metstruckplod: metstruckplodModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = metstruckplodRecord
      ? { ...initialValues, ...metstruckplodRecord }
      : initialValues;
    setTrucknumber(cleanValues.trucknumber);
    setEnginehours(cleanValues.enginehours);
    setDate(cleanValues.date);
    setOperatorname(cleanValues.operatorname);
    setHoursworked(cleanValues.hoursworked);
    setShift(cleanValues.shift);
    setCrew(cleanValues.crew);
    setErrors({});
  };
  const [metstruckplodRecord, setMetstruckplodRecord] = React.useState(
    metstruckplodModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Metstruckplod, idProp)
        : metstruckplodModelProp;
      setMetstruckplodRecord(record);
    };
    queryData();
  }, [idProp, metstruckplodModelProp]);
  React.useEffect(resetStateValues, [metstruckplodRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Metstruckplod.copyOf(metstruckplodRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MetstruckplodUpdateForm")}
      {...rest}
    >
      <TextField
        label="Trucknumber"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || metstruckplodModelProp)}
