...
export default function DevelopmentdrillingUpdateForm(props) {
  const {
    id: idProp,
    developmentdrilling: developmentdrillingModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = developmentdrillingRecord
      ? { ...initialValues, ...developmentdrillingRecord }
      : initialValues;
    setHeading(cleanValues.heading);
    setHoles(cleanValues.holes);
    setSplitsetbolts(cleanValues.splitsetbolts);
    setTunnelprofilewidth(cleanValues.tunnelprofilewidth);
    setTunnelprofileheight(cleanValues.tunnelprofileheight);
    setMeshsheet(cleanValues.meshsheet);
    setScalinghours(cleanValues.scalinghours);
    setTunnelprofiledistance(cleanValues.tunnelprofiledistance);
    setErrors({});
  };
  const [developmentdrillingRecord, setDevelopmentdrillingRecord] =
    React.useState(developmentdrillingModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Developmentdrilling, idProp)
        : developmentdrillingModelProp;
      setDevelopmentdrillingRecord(record);
    };
    queryData();
  }, [idProp, developmentdrillingModelProp]);
  React.useEffect(resetStateValues, [developmentdrillingRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Developmentdrilling.copyOf(developmentdrillingRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DevelopmentdrillingUpdateForm")}
      {...rest}
    >
      <TextField
        label="Heading"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || developmentdrillingModelProp)}
