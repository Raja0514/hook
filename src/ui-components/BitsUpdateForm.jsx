...
export default function BitsUpdateForm(props) {
  const {
    id: idProp,
    bits: bitsModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = bitsRecord
      ? { ...initialValues, ...bitsRecord }
      : initialValues;
    setBitssize(cleanValues.bitssize);
    setBitsused(cleanValues.bitsused);
    setBitsdamage(cleanValues.bitsdamage);
    setBitslost(cleanValues.bitslost);
    setErrors({});
  };
  const [bitsRecord, setBitsRecord] = React.useState(bitsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Bits, idProp)
        : bitsModelProp;
      setBitsRecord(record);
    };
    queryData();
  }, [idProp, bitsModelProp]);
  React.useEffect(resetStateValues, [bitsRecord]);
  const validations = {
    bitssize: [],
    bitsused: [],
    bitsdamage: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Bits.copyOf(bitsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BitsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Bitssize"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || bitsModelProp)}
