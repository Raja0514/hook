...
export default function ReamersUpdateForm(props) {
  const {
    id: idProp,
    reamers: reamersModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = reamersRecord
      ? { ...initialValues, ...reamersRecord }
      : initialValues;
    setReamerssize(cleanValues.reamerssize);
    setReamersused(cleanValues.reamersused);
    setReamersdamage(cleanValues.reamersdamage);
    setReamerslost(cleanValues.reamerslost);
    setErrors({});
  };
  const [reamersRecord, setReamersRecord] = React.useState(reamersModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Reamers, idProp)
        : reamersModelProp;
      setReamersRecord(record);
    };
    queryData();
  }, [idProp, reamersModelProp]);
  React.useEffect(resetStateValues, [reamersRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Reamers.copyOf(reamersRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ReamersUpdateForm")}
      {...rest}
    >
      <TextField
        label="Reamerssize"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || reamersModelProp)}
