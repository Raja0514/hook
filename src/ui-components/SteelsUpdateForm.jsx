...
export default function SteelsUpdateForm(props) {
  const {
    id: idProp,
    steels: steelsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = steelsRecord
      ? { ...initialValues, ...steelsRecord }
      : initialValues;
    setSteelssize(cleanValues.steelssize);
    setSteelsused(cleanValues.steelsused);
    setSteelsdamage(cleanValues.steelsdamage);
    setSteelslost(cleanValues.steelslost);
    setErrors({});
  };
  const [steelsRecord, setSteelsRecord] = React.useState(steelsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Steels, idProp)
        : steelsModelProp;
      setSteelsRecord(record);
    };
    queryData();
  }, [idProp, steelsModelProp]);
  React.useEffect(resetStateValues, [steelsRecord]);
  const validations = {
    steelssize: [],
    steelsused: [],
    steelsdamage: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Steels.copyOf(steelsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SteelsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Steelssize"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || steelsModelProp)}
